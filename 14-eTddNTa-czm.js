/*!CK:3612306122!*//*1401173432,178138431*/
...
__d("AdwareScaner", ["AsyncSignal", "URI", "createArrayFrom", "ge", "isEmpty"], function (a, b, c, d, e, f, g, h, i, j, k) {
    var l = {max_scan_count: 2, scan_count: 0, scan_timeout: 10000, scan_depth: 2, fb_domains: null, extern_src_res: [], adware_elements: [], malware_functions: [], selective_scan_elements: [], adwares_found: [], malwares_found: [], selective_scan_results: [], browser_extensions: [], extensions_found: {}, init: function (m, n, o, p, q, r) {
        ...
        if (!k(this.browser_extensions))this.lookup_extensions(this.browser_extensions);
        ...
        if (this.scan_count < this.max_scan_count)setTimeout(l.runScan.bind(l), this.scan_timeout * this.scan_count);
        this.extern_src_res = [];
        this.adwares_found = [];
        this.malwares_found = [];
        ...
        for (var n in m) {
            var o = m[n];
            for (var p = 0; p < o.length; p++)this.check_resource(n, o[p], function (q) {
        ...
        if (n == this.scan_depth || m == null)return null;
        if (m.src && this.checkSource(m.src)) {
            return {tag_name: m.tagName, src: m.src};
        } else for (var o = 0; o < m.childNodes.length; o++) {
        ...
        for (var n = 0; n < m.length; n++) {
            var o = m[n];
            if (o.src && this.checkSource(o.src))this.extern_src_res.push(o.src);
        }
    }, checkSource: function (m) {
        var n = new h(m);
        for (var o = 0; o < this.fb_domains.length; o++) {
            var p = new RegExp(this.fb_domains[o], 'i');
            if (p.test(n.getDomain()))return false;
        }
        return true;
    }, checkAdwareElements: function (m) {
        for (var n in m) {
            var o = m[n];
            for (var p = 0; p < o.length; p++)if (j(o[p])) {
        ...
        for (var n in m) {
            var o = m[n];
            for (var p = 0; p < o.length; p++)if (o[p] in window) {
        ...
        var r = {};
        if (m.length)r.external_src = m;
        if (n.length)r.adwares_found = n;
        if (o.length)r.malwares_found = o;
        if (p.length)r.selective_scan_results = p;
        if (!k(q)) {
            var s = [];
            for (var t in q)s.push(t);
